...
function ProformaDetail() {
  ...
  const bankAccountArray = contact.split(":");
  const getProforma = async (id) => {
    const response = await api.GetProforma(id);
    console.log(response);
    return response;
  };

  const mutation = useMutation(getProforma, {
    onSuccess: (response) => {
      setProformaDetail(response);
      console.log(eachOrder, "order");
      console.log(eachProforma[0], "proforma");
    },
  });
  useEffect(() => {
    try {
          ...
          <table class="mb-3 min-w-full">
            ...
            <tbody class="divide-y divide-gray-300">
              {eachOrder.map((items, index) => (
                <tr onClick={() => handleRowClick(eachOrder[index].id)}>
